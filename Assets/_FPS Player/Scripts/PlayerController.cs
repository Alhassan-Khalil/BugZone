...
public enum Status { idle, walking, crouching, sprinting, sliding, climbingLadder, wallRunning, vaulting, grabbedLedge, climbingLedge }
...
public class PlayerController : MonoBehaviour
{
    ...
    void UpdateMovingStatus()
    {
        ...
        {
            if (playerInput.RawInputMovement.magnitude > 0.02f)
    ...
    {
        bool sprint = false;
        sprint = (playerInput.ToggleSprint && playerInput.RawInputMovement.y > 0);
    ...
    void DefaultMovement()
    {
        ...
        movement.Move(playerInput.RawInputMovement, isSprinting(), isCrouching());
        if (movement.grounded && playerInput.JumpInput)
        {
    ...
    void CheckCrouching()
    {
        ...
        if (playerInput.ToggleSprint)
        {
            Uncrouch();
            return;
        }

        if (playerInput.ToggleCrouch)
        {
            if (status != Status.crouching)
                Crouch(true);
            else
